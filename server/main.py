...
def get_pagination_response(
    ...
    return {
        'previous': f'{url}?offset={prev_offset}&pagesize={prev_pagesize}' \
            if current_offset > 0 else None,
        'next': f'{url}?offset={next_offset}&pagesize={next_pagesize}' \
...
def list_word(level: Optional[int] = Query(None, gt=0), pagination_parameters: dict = Depends(pagination_parameters)):
    ...
    return {
        ...
        'total': total,
        'pagination': get_pagination_response(f'{BASE_URL}/words', total, offset, pagesize)
    }
