...
function SuraSelect({ valueClassName, selectedSuraNum, onSelectSura }) {
  ...
  return (
    <div
      className="SuraSelect"
      onClick={() => setIsOptionsExpanded(!isOptionsExpanded)}
    >
      <div className={valueClassName}>{selectedSuraNum}</div>
      <div
        className={`SuraSelect-Options${isOptionsExpanded ? "" : "-hidden"}`}
      >
        {
          ...
          suraList.map(({ id, name, meaning }) => (
            <div
              key={id}
              className="SuraSelect-Option"
              onClick={() => onSelectSura(id)}
            >
